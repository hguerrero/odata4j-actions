...
public class EntitiesRequestResource extends BaseResource {
  ...
  @POST
  @Produces({ ODataConstants.APPLICATION_ATOM_XML_CHARSET_UTF8, ODataConstants.APPLICATION_JAVASCRIPT_CHARSET_UTF8 })
  public Response createEntity(
    ...
    ODataContext odataContext = ODataContextImpl.builder()
      ...
      // TODO: workaround, document this.
      // Skip this check for Infinispan OData server, I hope we don't need any mediaLinkEntries
      // So there is no need to define structural entities in a producer (which is pain)
...
//    }
  ...
  @PUT
  @Produces({ ODataConstants.APPLICATION_JAVASCRIPT_CHARSET_UTF8 })
  public Response functionCallPut(
    ...
    if (producer.getMetadata().containsEdmFunctionImport(functionName)) {
      ...
      QueryInfo query = QueryInfo.newBuilder().setCustomOptions(OptionsQueryParser.parseCustomOptions(uriInfo)).build();
      response = FunctionResource.callFunction(ODataHttpMethod.PUT, httpHeaders, uriInfo, securityContext, producer, functionName, format, callback, query, payload);
    } else {
  ...
  public Response functionCallDelete(
    ...
    if (producer.getMetadata().containsEdmFunctionImport(functionName)) {
      ...
      QueryInfo query = QueryInfo.newBuilder().setCustomOptions(OptionsQueryParser.parseCustomOptions(uriInfo)).build();
      response = FunctionResource.callFunction(ODataHttpMethod.DELETE, httpHeaders, uriInfo, securityContext, producer, functionName, format, callback, query, payload);
    } else {
      throw new NotFoundException(functionName);
    }
...
//      ODataConstants.TEXT_JAVASCRIPT_CHARSET_UTF8,
  ...
  protected Response getEntitiesImpl(
    ...
    if (producer.getMetadata().containsEdmFunctionImport(entitySetName)) {
      ...
      // functions that return collections of entities should support the
      // same set of query options as entity set queries so give them everything.
      return FunctionResource.callFunction(ODataHttpMethod.GET, httpHeaders, uriInfo, securityContext, producer, entitySetName, format, callback, query, null);
    }
    ...
    if (isCount) {
      ...
      ODataVersion version = ODataVersion.V3;
