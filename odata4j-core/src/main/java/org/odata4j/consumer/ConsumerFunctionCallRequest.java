...
//public class ConsumerFunctionCallRequest<T extends ODataClientResponse>
public class ConsumerFunctionCallRequest<T extends Object>
...
//    Enumerable<OObject> results;
    Enumerable<ODataClientResponse> results = null; // workaround
    if (function.getReturnType() == null){
      doRequest(request);
      results = Enumerable.empty(null);
    } else if (function.getReturnType() instanceof EdmCollectionType) {
       throw new UnsupportedOperationException("This is workarounded in ConsumerFunctionCallRequest");
...
//          });
  ...
  @Override
  public OFunctionRequest<T> pByteArray(String name, byte[] value) {
     return parameter(name, OSimpleObjects.create(EdmSimpleType.BINARY, value));
  }
...
//          );
//
//       // get FeedReader returns new BOMWorkaroundReader(new InputStreamReader(textEntity, Charsets.Upper.UTF_8));
...
//  }
   ...
   /**
    * Modified version of client for better performance.
    * This is not parsing Feed by any parser and just return raw data.
    *
    * @param request
    * @return
    * @throws ODataProducerException
    */
   private ODataClientResponse doRequest(ODataClientRequest request) throws ODataProducerException {
      ODataClientResponse response = getClient().callFunction(request);
      ...
      if (function.getReturnType() == null) {
         return null;
          ...
          // its already properly formatted from producer function call
